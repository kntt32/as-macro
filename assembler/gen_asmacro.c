...
static ParserMsg Argument_parse_storage_bound(inout Parser* parser, inout Argument* argument) {
    ...
    if(!Parser_start_with(parser, "imm") && ParserMsg_is_success(Storage_parse(&parser_copy, NULL, NULL, &argument->type, &argument->storage.storage))) {
...
static ParserMsg Asmacro_parse_proc(inout Parser* parser, out Asmacro* asmacro) {
    ...
    asmacro->type = Asmacro_UserMacro;
    asmacro->body.user_macro = block_parser;
...
static ParserMsg Asmacro_parse_encoding(inout Parser* parser, out Asmacro* asmacro) {
    ...
    asmacro->type = Asmacro_AsmMacro;
    PARSERMSG_UNWRAP(
        AsmEncoding_parse(&paren_parser, &sizes, &asmacro->body.asm_macro),
        (void)NULL
    );
...
void Asmacro_print(in Asmacro* self) {
    ...
    switch(self->type) {
        case Asmacro_AsmMacro:
            printf(".asm_macro: ");
            AsmEncoding_print(&self->body.asm_macro);
            break;
        case Asmacro_UserMacro:
            printf(".user_macro: ");
            Parser_print(&self->body.user_macro);
            break;
...
Asmacro Asmacro_clone(in Asmacro* self) {
    ...
    switch(self->type) {
        case Asmacro_AsmMacro:
            asmacro.body.asm_macro = AsmEncoding_clone(&self->body.asm_macro);
            break;
        case Asmacro_UserMacro:
            asmacro.body.user_macro = self->body.user_macro;
            break;
        case Asmacro_FnWrapper:
...
void Asmacro_free(Asmacro self) {
    ...
    switch(self.type) {
        case Asmacro_AsmMacro:
            AsmEncoding_free(self.body.asm_macro);
            break;
        case Asmacro_UserMacro:
            break;
        case Asmacro_FnWrapper:
